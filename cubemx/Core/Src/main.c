...
/* USER CODE BEGIN WHILE */
	...
	uint32_t state_curr2, state_prev2 = LL_GPIO_IsInputPinSet(BTN_2_GPIO_Port, BTN_2_Pin),
		state_curr1, state_prev1 = LL_GPIO_IsInputPinSet(BTN_1_GPIO_Port, BTN_1_Pin);
	uint32_t sum = 0;
	int32_t enc_delta = 0;
	while (1) {
		//for(volatile uint32_t delay = 0; delay < 100000; delay++);
		
		//printf(ESCAPE_GREEN "tick%lu" ESCAPE_NORM "\n", TIM2->CNT);
